...
\begin{primer}
 ...
 Obratno vzemimo $N$ podgrupo edinko grupe $G$. In dfinirajmo relacijo $\theta$ takole: $a \theta b \Leftrightarrow 
 a b^{-1} \in N$. Pokažimo, da je tako definirana relacija kongruenca. To je res ekvivalnečna relacija: $(r)$ $aa^{-1}=
 1 \in N$, $(s)$ če je $ab^{-1} \in N$ za vsaka $a, b \in N$ je tudi $ba^{-1} \in N$ in še (t) če sta $ab^{-1}$ in
 $bc^{-1}$ iz $N$ je tudi $ab^{-1} b c^{-1}= a c^{-1} \in N$. Pokažimo še, da je $\theta$ kongruenca.
